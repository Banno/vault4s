...
/** A String wrapper class, to ensure that the string inside is a valid Base64
  * string, as those used in Vault to represent plaintext and context.
  */
...
object Base64 {
  ...
  /** A cost-free predicate to check if a String is a valid encoding outcome as
    * described in https://tools.ietf.org/html/rfc4648#section-4.
    *
    * Meaning: its length is a multiple of 4, and all its characters are
    * letters, digits, '+' or '/'. Except at the end, where either: a) the last
    * two characters can both be padding `=``; or b) only the last character is
    * padding; or c) no character is padding.
    */
  private[transit] def isBase64(str: String): Boolean =
    (str.length & 3) == 0 && {
      (str.length - str.count(isBase64)) match {
        ...
        case 2 =>
          isPadding(str.charAt(str.length - 1)) && isPadding(
  ...
  private def isBase64(char: Char): Boolean =
  ...
  def fromString[F[_]](
      str: String
  )(implicit F: ApplicativeError[F, String]): F[Base64] =
    if (isBase64(str)) F.pure(new Base64(str))
    else
      F.raiseError(
        s"""The string "$str" is not a valid Base-64 encoded literal"""
      )
